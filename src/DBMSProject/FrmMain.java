...
import static utils.GlobalData.per;
...
public class FrmMain extends javax.swing.JFrame {
    ...
    FrmViewBookGuest frmViewBookGuest = new FrmViewBookGuest();
    //Create a map for all the forms
    Map<String, JInternalFrame> forms = new HashMap<>();
    Map<String, Boolean> memberLogin = new HashMap<>();
    ...
    public FrmMain() {
        ...
        forms.put("frmAddBook", frmAddBook);
        forms.put("frmViewBook", frmViewBook);
        forms.put("frmViewBookGuest", frmViewBookGuest);
        roles.put("frmLogin", Arrays.asList(1, 2));
        roles.put("frmAddBook", Arrays.asList(1));
        roles.put("frmViewBook", Arrays.asList(1));
        roles.put("frmViewBookGuest", Arrays.asList(1, 2));
        memberLogin.put("frmViewBookGuest", Boolean.TRUE);
        memberLogin.put("frmAddBook", Boolean.FALSE);
        memberLogin.put("frmViewBook", Boolean.FALSE);
    ...
    private void showForm(String name, boolean checkLogin){
        ...
        if (checkLogin && GlobalData.per == null) {
            //Check for authorization here
            showForm("frmLogin", false);
        } else {
            try {
                try {
                    //check the role of the currently logged in user (current user)
                    if (per != null && roles.containsKey(name)) {
                        List<Integer> allowedRoles = roles.get(name);
                    boolean hasPermission = false;
                    for (int allowedRole : allowedRoles) {
                    if (per.getRole() == allowedRole) {
                    hasPermission = true;
                    break;
        }
    }
    ...
    if (!hasPermission) {
        //showForm("frmLogin", false);
        JOptionPane.showMessageDialog(this, "Invalid permissions");
        return;
    }
}
                ...
                forms.get(name).setVisible(true);
                forms.get(name).setSelected(true);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                ...
                ja.setSize(Toolkit.getDefaultToolkit().getScreenSize());
                ja.setVisible(true);
            }
        });
    }
